...
public class Worker extends RecursiveAction {
	...
	protected void computeDirectly() {
		...
		try {
			...
			if(foundTokens) {
				WorkPoolJF.addPartialResult(wordsFreq);
			}
			f.close();
	...
	protected void compute() {
		if (mEnd - mStart < WorkPoolJF.processingLength) {
			computeDirectly();
			return;
		}
		long split = (mEnd - mStart) / 2;

		invokeAll(new Worker(file, mStart, mStart + split),
				new Worker(file, mStart + split + 1, mEnd));
	}
